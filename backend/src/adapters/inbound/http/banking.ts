...
import { bankSurplus, applyBanked } from '../../../core/application/usecases/usecases'
...
const repo: any = useInMemory ? require('../../outbound/inmemory/routeRepo').InMemoryRouteRepo && new (require('../../outbound/inmemory/routeRepo').InMemoryRouteRepo)() : new PostgresRouteRepo()
...
router.post('/bank', async (req, res) => {
  ...
  const surplus = bankSurplus(routes, emissionsByRoute)
...
router.post('/apply', async (req, res) => {
  ...
  const routes = await repo.list()
  const allocation = applyBanked(banked, routes)
